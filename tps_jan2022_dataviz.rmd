...
train_final %>% mutate(year = factor(year),country = as.factor(country)) %>% ggplot(aes(year,num_sold,colour=country)) +geom_boxplot() + ylab("Units Sold") + ggtitle("Boxplot of Units Sold")
...
for(i in 1:nrow(params)){
  lineplot_list[[i]] = train_final %>% mutate(country = as.factor(country)) %>% filter(product== params[i,1] & store==params[i,2]) %>% ggplot(aes(date,num_sold)) +geom_line(aes(color=country),lwd=0.5) + ylab("Units Sold") + ggtitle(paste0("Line plot of ",params[i,1],"s sold at ",params[i,2])) + theme(plot.title = element_text(size = 8))
  
}

marrangeGrob(lineplot_list,nrow=2,ncol=1)

#looking at these plots we can see that there's definitely seasonality, we can go in depth for each product to get a clearer picture.
```
...
another_list[[i]] = train_final %>% filter(product == params[i,1] & store== params[i,2] & country ==params[i,3]) %>% group_by(month,year) %>% summarise(avg_sales = mean(num_sold)) %>% mutate(year = as.factor(year)) %>% ggplot(aes(month,avg_sales,color=year)) + geom_point() + geom_line() + ylab("Average Sale") +ggtitle(paste0("Avg Sale of ",params[i,1]," per Month \n ",params[i,2],":",params[i,3])) + theme(plot.title = element_text(size = 8))
}
